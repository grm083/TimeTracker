...
var selectedHours = "";
var selectedControl, teamCode = '', teamWorkType = '', ALMprojectType = 'other', productionProjectType = 'production support';
...
function initializevalues(tempteamCode, tempTeamWorkType) {
                ...
                for (var r = 0; r <= rowCount; r++) {
                    ...
                    if (successData != null && successData != '' && successData.length > 0) {
                        for (var i = 0; i < successData.length; i++) {
                            //debugger;
                            var applicationid = 0, projectItemId = 0, workTypeId = 0, projectId = 0;
                            var workItem;
                            ...
                            //$('.projectRow_' + r).find("workitem").each(function (index, item) {
                            //    if ($(item).hasClass("workitem"))
                            //        workItem = $(item).val();
                            //});

                            var tempApplicationid = 0, TempProjectItemId = 0, TempWorkTypeId = 0, tempProjectId = 0;
                            var tempworkItem;
            ...
            success: function (successData) {
                ...
                if (successData != null && successData != '' && successData.length > 0) {
                    ...
                    if (tempTeamWorkType != undefined && tempTeamWorkType != '') {
                        $("#selectWorkType").val(tempTeamWorkType);
                    }
                    $(".workitem").prop('disabled', true);

                    if (tempteamCode != null && tempteamCode == 'PS') {
                        $('[name=selectProject] option').filter(function () {
                            return ($(this).text() == 'Production Support'); //To select Blue
                        }).prop('selected', true);
                        $("[name=selectProject]").trigger("change");
                    }
                }
                closeModal();
            },
...
function SaveTimeSheetRecord(classname) {
    ...
    var FinalRecord = { "UserId": employeeId, "LogonName": "", "TeamCode": 'DEV', "TimeEntry": TimeSheetRecordArray }
...
$(document).ready(function () {
    ...
    $("#datepicker_start").datepicker(
        {
            ...
            onSelect: function (selectedDate) {
                ...
                var firstday = 7 - firstDate.getDay();
                if (firstday != 0) {
                    firstDate.setDate(firstDate.getDate() - firstDate.getDay());
                }
                firstDate = firstDate.getTime();
                if ((new Date(selectedDate).getTime() < firstDate)) {
                    //$('#btnPanel').hide();
                    //$('input[type="text"], input[type="submit"], input[type="button"], select').prop("disabled", true);
                    $('.rowDeleteClick').hide();
                }
                else {
                    $('#btnPanel').show();
                    $('.rowDeleteClick').show();
                    $('input[type="text"], input[type="submit"], input[type="button"], select').prop("disabled", false);
                }
    ...
    $("#btnSubmit").click(function () {
        ...
        submitted = true; currentRowClass = '';
        $('#commentSection').hide();
...
function openModal() {
    ...
    setTimeout("$body.addClass('loading')", 1000);
}
...
function CrerateNewProjectRow() {
    ...
    var html = "<div class='projectRow_" + productRowCount + " divTableRow'>" +
        ...
        "<select class='form-control clsSelectProject ddValue ProjectId' title=Select Project' name='selectProject_" + productRowCount + "'><option value=''>Select Project</option>" +
        projecrList +
        "</select > " +
        "</div>" +
        "<div class='col-md-2 divTableCell'>" +
        "<select class='form-control clsSelectProjectType ddValue ProjectItemId' title='Select Project Item' name='selectProjectType_" + productRowCount + "'><option value=''>Select Project Item</option></select>" +
        "</div>" +
        "<div class='col-md-2 divTableCell'>" +
...
function BindInitialSelectBox() {
    ...
    var strWorkType = "";
    strWorkType = "<option value=''>Select Work Type</option>";
    for (i = 0; i < workTypeArray.length; i++) {
    ...
    var strProjectType = "<option value=''>Select Project Item</option>";
    $(".clsSelectProjectType ").empty();
    $("#selectWorkType ").empty();
    $("#selectProject").append(strProject);
    $("#selectWorkType").append(strWorkType);
    $(".clsSelectProjectType ").append(strProjectType);
